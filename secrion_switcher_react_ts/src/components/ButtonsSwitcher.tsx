...
interface ButtonSwitcherProps {
  scrollToStory: (index: number) => void;
  currentIndex: number;
  stories: Story[];
}
...
const ButtonsSwitcher = ({
  ...
  return (
    <div className={"btn_wrapper"}>
      <div
        onClick={hasPrev ? () => scrollToStory(currentIndex - 1) : undefined}
        className={`go_to-prev ${!hasPrev ? "disabled" : ""}`}
      >
        <svg
          fill="#000000"
      ...
      <div
        onClick={hasNext ? () => scrollToStory(currentIndex + 1) : undefined}
        className={`go_to-next ${!hasNext ? "disabled" : ""}`}
      >
        <svg
          fill="#000000"
          xmlns="http://www.w3.org/2000/svg"
